...
def _train_input_fn():
    ...
    return input_fn(record_file="recordFile/dataTrain.record", batch_size=32, num_epochs=50, shuffle=True)


def _eval_input_fn():
    """Function for evaluating."""
    return input_fn(
        record_file="recordFile/dataTest.record",
        batch_size=2,
        num_epochs=1,
        shuffle=False)
...
def _predict_input_fn():
    ...
    return input_fn(
        record_file="recordFile/dataValidation.record",
        batch_size=2,
        num_epochs=1,
        shuffle=False)
...
def main(unused_argv):
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
        ...
        estimator.train(input_fn=_train_input_fn, steps=4500)
